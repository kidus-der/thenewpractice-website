...
| `ui.ts` | `UI` | Scroll cue (l.18); skip link is ours | Hand-written (Task 1) |
...
- **`serviceSchema`** — `slug`, `order`, `title`, `intro`, `treatsHeading?`/`treats?`, `mayIncludeHeading?`/`mayInclude?`, `outro?`, `definitions?`, `subsections?`. Where a service has both lists (Addiction Treatment only) the document places *may include* before *treats*; render it that way.
