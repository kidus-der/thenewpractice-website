...
- **Instruments of care, abstracted.** A glass of water. Never a syringe, chart, monitor, or anything that reads *hospital*.
...
**Also forbidden, specific to this brief:** turquoise-water resort photography, white-sand beach clichés, hammocks, palapa-with-cocktail, Maya ruins as backdrop, resort branding, anything that reads *holiday*. The client is not on holiday. And no alpine, Nordic, or temperate-forest landscape — it contradicts the location outright.
...
| **Ambient gradient** *(home hero only, desktop, gated)* | `@shadergradient/react` water-plane in canopy / canopy-soft / stone at 0.35 opacity behind the video, `grain="off"` | Depth behind the surf without a second colour. Never brass. Removable in one file. See `docs/04` §8, `docs/07`. |
...
- Trails with a lerp of `0.28` — weighted, but never *behind* the pointer
...
- **Lerp at `0.28`, not lower.** Below that the dot reads as *trailing* rather than *weighted*, and on the 120Hz displays this audience uses it reads as broken.
