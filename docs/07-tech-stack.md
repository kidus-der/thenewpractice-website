...
| Content                | **Typed TS modules per collection with Zod schemas** (`src/content/**`), generated from the client document by `scripts/ingest-content.mjs`                                                                                       | Modules export plain objects typed by the schemas' inferred types; `content.checks.ts` parses every module against its schema in `npm test` and `npm run content:check`, so Zod never enters a client bundle through the content layer (Task 20). Shaped so a CMS (recommendation: Sanity, on the client's account) can replace the module source after template approval without touching templates.                                                                                                                                                             |
...
Dev only: `eslint ^9` + `eslint-config-next`, `prettier` + `prettier-plugin-tailwindcss`, `sharp` (asset pipeline), `@types/*` (`@types/node ^24` for the Node 26 runtime, `@types/three 0.186.0`), `vitest ^5` + `@vitest/coverage-v8 ^5` (Vite 8 / oxc; the JSX runtime is set through `oxc.jsx` in `vitest.config.ts`) + `@testing-library/react` + `jsdom`, `@playwright/test` + `@axe-core/playwright`, `@lhci/cli` (task 4; Task 20 bumped Vitest and `@types/node`). Bundle analysis is Next's own `next experimental-analyze` (Turbopack); no analyser package is installed.

No icon library, no utility grab-bag, no date library, no animation helper on top of GSAP or Motion.

## Configuration notes

**Fonts** — `next/font/google`, self-hosted at build, `display: 'swap'`, subset `latin` + `latin-ext`. Preload the display face only, at the one weight the stylesheets use (400, upright and italic).

**Images** — `next/image`; every image has explicit `sizes` measured against its rendered width (docs/09 §Measured budgets) and a `blurDataURL` from `media.ts`; per-frame `quality` from `src/lib/plates.ts` with `images.qualities` in `next.config.ts` as the allowlist. The hero poster gets `priority`; nothing else does.
...
- Input is validated at the boundary with Zod: the form, the server action, the environment, and the content layer in `content.checks.ts` (never at module scope in a content module, which would ship the schemas to the client).
