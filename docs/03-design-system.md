...
> **Rule:** no component contains a raw hex, px, ms or easing value. If the token you need is not here, add it to `src/app/globals.css` *and* here in the same change, then use it. `src/motion/tokens.ts` mirrors the motion tokens for GSAP and Motion.
...
| `--accent` | `--c-brass` | `--c-brass` *(deliberately does not flip)* |
...
| **Mono** | *(none)* | — | No monospace on this site. Numerals come from Jost's tabular figures. |
...
| Case | Sentence case. **All-caps in the serif only for the wordmark** and the client's own all-caps titles (*A NEW APPROACH TO WELLBEING*). | Caps permitted only at `--t-eyebrow` |
...
| Italic | Serif italic is the *only* emphasis mechanism on this site. No bold in the display face. The client's markdown emphasis is stripped on ingestion; italics return only where a template chooses them. | |
...
There are exactly three interactive treatments on this site. Do not invent a fourth. Navigation links, index rows, form submits and the *Enquire* CTA all resolve to one of them.
