...
`/our-process` (`src/app/our-process/page.tsx`): twelve sections, so the sticky index shows from 1024px; `hero-cenote-poster` on _Arriving at The New Practice_ and no other image; _A Typical Day_ on sand with `bodies`; rail About ← Our Process → Clinical Services; `assertSectionIds()` guards both ids.

`/a-personal-message`: `variant="letter"`, the client's eyebrow _THANK YOU_ on the title page, the letter recomposed through `letterSection()`, bone throughout. `/fees`: `variant="letter"`, the client's heading _Cost_ as the `h1` (nav and metadata say _Fees_, ledger Task 10 triage), the one statement, nothing else. Neither route is in the primary navigation, so `prevNextFor()` gives them no rail and the enquire band closes them.

`/privacy` and `/terms`: `src/templates/LegalStub.tsx` over `pages/legal.ts` (hand-written, every string `PLACEHOLDER — `, four headings each with one sentence saying the text is pending counsel; `content.checks.ts` requires the prefix). `buildMetadata({ noIndex: true })`, and both routes sit in `NOINDEX_ROUTES` (`nav.ts`), which `sitemapEntries()` filters out. Off production (`isIndexable()` false: development and staging) the title page's eyebrow is `UI_STAGING.copyPending`, _Copy pending client review_, in place of the page's own; on production it is not rendered. When counsel's text arrives: swap the module, drop `noIndex`, remove the route from the set, delete the stub file.
...
| Block | Spec as shipped |
| --- | --- |
| `PageIntro` | As T2, with the optional `notice` line above the lockup: `UI_RESIDENCES.copyPending` in the display italic at `--t-small`, `--fg-muted`, rendered by the route only when `env().SITE_ENV !== 'production'` — staging and development show the flag, production cannot. |
