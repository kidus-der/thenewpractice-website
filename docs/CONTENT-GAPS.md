...
| G4 | **Hero video and all photography.** The document briefs a 15-second surf-to-jungle film (l.10) and nothing else. | Home hero, every plate, team portraits | Stock, licence-free media per the owner decision (Tasks 2a/2b, `design/ASSETS.md`). Team pages use a generated placeholder plate (Task 14, `PortraitPlaceholder`): a 3:4 sand frame with one vertical hairline and the gold point at the golden section, labelled _Portrait to follow_ — no face, no silhouette, no initials (docs/02: the human presence only obliquely). A portrait replaces it per member by adding a `media.ts` key and passing `portrait` in `team/[slug]/page.tsx`; the template does not change. | **Client:** commission or supply the hero film, residence photography and eleven portraits. |
...
| G6 | **Per-questionnaire interpretation lines.** The shared block (l.1024) has one interpretation, written for alcohol: "Higher scores suggest increasing levels of problematic alcohol use." | `/self-assessment/[slug]` result panel | The alcohol questionnaire gets the full sentence. The other nine get only its generic second sentence, "Moderate or severe scores indicate it may be helpful to seek professional support." Nothing is invented. | **Client:** write one interpretation line per questionnaire (nine needed). |
